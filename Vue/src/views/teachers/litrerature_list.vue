<template>
    <div class=" mt-10">
        ...
        <div class="mt-5 mb-4 flex items-center justify-between">
            <router-link class="btn-all" :to="{ name: 'app.literature.create', params: { id: props.id } }">
        ...
        <div class="bg-white p-6 rounded shadow animate-fade-in-down overflow-auto">
            ...
            <div class="bg-green-500 text-white rounded py-4 text-center mb-3" v-if="teacherStore.msg_dsuccess">
                <div class="flex items-center justify-between px-10">
                    <div
                        class="hover:bg-green-400 text-white rounded-full h-8 w-8 cursor-pointer flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" @click="teacherStore.msg_dsuccess = ''" fill="none"
            ...
            <div class="bg-red-500 text-white py-4 rounded text-center mb-3" v-if="teacherStore.msg_dwrang">
                <div class="flex items-center justify-between px-10">
                    <div
                        class="hover:bg-red-300 text-white rounded-full h-8 w-8 cursor-pointer flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" @click="teacherStore.msg_dwrang = ''" fill="none"
                            viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
                        </svg>
                    </div>
            ...
            <div class="flex justify-between border-b-2 pb-3">
                ...
                <div>
                    <input v-model="search" @change="getLiteratures(null)"
                        class="appearance-none relative block w-48 px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
                        placeholder="جستجوی بر اساس تاریخ عنوان,">
                </div>
                <div class="flex items-center">
                    <span class="whitespace-nowrap mr-3">هر صفحه</span>
                    &nbsp;
                    <select dir="ltr" @change="getLiteratures(null)" v-model="perPage"
                    ...
                    &nbsp;
                    <span class="ml-3">پیداشد {{ literatures.total }} دیتا</span>
                </div>
            </div>
            <table class="table-auto w-full">
                <thead>
                    <tr>
                        ...
                        <TableHeaderCell field="publisher" :sortDirection="sortDirection" :sortField="sortField"
                            @click="sortUsers('publisher')">
                            ژورنال
                        </TableHeaderCell>
                ...
                <tbody v-else>
                    <tr v-for="(literature, index) of literatures.data">
                        ...
                        <td class="border-b p-2 ">
                            <a :href="literature.publisher" target="__blank" class="hover:text-blue-600 ">
                                ...
                                <transition enter-active-class="transition duration-100 ease-out"
                                    ...
                                    <MenuItems
                                        ...
                                        <div class="px-1 py-1">
                                            ...
                                            <router-link :to="{ name: 'app.teacher.edit', params: { id: literature.id } }"
                                                :class="[
                                        ...
                                        <div class="px-1 py-1">
                                            <MenuItem v-slot="{ active }">
                                            <button @click="deleteTeacher(literature.id)" :class="[
                                                active ? 'bg-blue-800 text-white' : 'text-gray-900',
                                                'group flex w-full items-center rounded-md gap-3 px-2 py-2 text-sm',
                                            ]">
            ...
            <div v-if="!literatures.loading" class="flex justify-between items-center mt-5" dir="ltr">
                <div v-if="literatures.data">
                    نمایش از {{ literatures.from }} تا {{ literatures.to }}
                </div>
                <nav v-if="literatures.total > literatures.limit"
                    class="relative z-0 inline-flex justify-center rounded-md shadow-sm -space-x-px"
                    aria-label="Pagination">
                    <!-- Current: "z-10 bg-indigo-50 border-indigo-500 text-indigo-600", Default: "bg-white border-gray-300 text-gray-500 hover:bg-gray-50" -->
                    <a v-for="(link, i) of literatures.links" :key="i" :disabled="!link.url" href="#"
                        ...
                        :class="[
                            ...
                            i === 0 ? 'rounded-l-md' : '',
                            i === literatures.links.length - 1 ? 'rounded-r-md' : '',
...
const search = ref('');
const literatures = computed(() => teacherStore.Literatures);
...
function getForPage(ev, link) {
    ...
    getLiteratures(link.url)
}

function getLiteratures(url = null) {
    teacherStore.getLiteratures({
        id: route.params.id,
        url,
        search: search.value,
